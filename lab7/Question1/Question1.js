function Student(studentId){
    this.studentId = studentId;
    this.answers =[];

}

Student.prototype.getStudentId = function(){
    return this.studentId;
}

Student.prototype.addAnswer = function(question){
    this.answers.push(question)
}


function Question(qid, answer){
    this.qid = qid;
    this.answer = answer;
}

Question.prototype.getQid = function(){
    return this.qid;
}

Question.prototype.getAnswer = function(){
    return this.answer
}

Question.prototype.checkAnswer = function (givenAnswer){
    return this.answer === givenAnswer
}

function Quiz(questions, students){
    this.questions = new Map();
    questions.forEach(q=>{
        this.questions.set(q.getQid(), q)
    });
    this.students = students;
}

Quiz.prototype.scoreStudentBySid = function(sid){
    const student = this.students.find(s=> s.getStudentId() === sid);
    if(!student) return 0;

    let score = 0;
    student.answers.forEach(ans =>{
        const correctQuestion = this.questions.get(ans.getQid());
        if(correctQuestion && correctQuestion.checkAnswer(ans.getAnswer())){
            score++;
        }
    })
    return score;
}

Quiz.prototype.getAverageScore = function(){
    const total = this.students.reduce((sum,s) =>{
        return sum + this.scoreStudentBySid(s.getStudentId());
    }, 0)
    return total / this.students.length;
};

const student1 = new Student(10);
student1.addAnswer(new Question(2, 'a'));
student1.addAnswer(new Question(3, 'b'));
student1.addAnswer(new Question(1, 'b'));
const student2 = new Student(11);
student2.addAnswer(new Question(3, 'b'));
student2.addAnswer(new Question(2, 'a'));
student2.addAnswer(new Question(1, 'd'));
const students = [student1, student2];
const questions = [new Question(1, 'b'), new Question(2, 'a'), new
    Question(3, 'b')];
const quiz = new Quiz(questions, students);
let scoreforStudent10 = quiz.scoreStudentBySid(10);
console.log(scoreforStudent10); //Expected Result: 3
let scoreforStudent11 = quiz.scoreStudentBySid(11);
console.log(scoreforStudent11); //Expected Result: 2
let average = quiz.getAverageScore();
console.log(average); //Expected Reuslt: 2.5